...
void main(List<String> arguments) async {
  ...
  while (true) {
    ...
    if (!colid.check(bola, court)) {
      //verifica se bateu e movimenta caso não
      move.moviment(
          bola, court, bola.realX, bola.realY); //não colidi em nada, movimento
    } else if (colid.check2(bola, court) == 1) {
      //colidi em cima
      ...
      bola.realX = -1;
      move.moviment(bola, court, bola.realX, bola.realY);
    } else if (colid.check2(bola, court) == 2) {
      //verifico se colidiu com a direita
      if ('|' == court.next(bola.posX, bola.posY + bola.height - 1, 2) &&
          '|' ==
              court.next(
                  bola.posX + bola.width, bola.posY + bola.height - 1, 2)) {
        //verifico se foi na lateral e não na raquete
        placar1++;
    ...
    } else if (colid.check2(bola, court) == 4) {
      //colidiu com a direita
      if ('|' == court.next(bola.posX, bola.posY, 4) &&
          '|' == court.next(bola.posX + bola.width, bola.posY, 4)) {
        placar2++;
        bola.objDelete(court);
        bola.objTomatrix(court, middleX, middleY);
      } else {
